...
<div class="d-flex justify-content-center mt-5">
    ...
    <ol class="breadcrumb nav-bread">
        ...
        ?>
        <li class="breadcrumb-item active" aria-current="page">Apply by Students</li>
    </ol>
    </nav>
</div>
    ...
    <div class="mt-5 table-overflow">
        <table class="table text-center">
            ...
            <?php 
                require_once 'database-connection.php';
                $mysqli = new mysqli(DB_HOST,DB_USER,DB_PASSWORD,DB_DATABASE);
                $qury = "SELECT apply_online.id,apply_online.name,apply_online.mobile,
                apply_online.email,course.course_name,course.course_fee, course.course_duration
                FROM apply_online,course WHERE apply_online.course_id = course.course_id";
                $count = 0;
                if($result=$mysqli->query($qury)){
                    if(mysqli_num_rows($result)>0){
                        while($row = $result->fetch_assoc()) {
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <td>
                                        <a class="btn btn-warning hold-gap" 
                                        href="hold-apply-online.php?id='.$id.'">Hold</a>
                                        <a class="btn btn-danger" 
                                        href="delete-apply-online.php?id='.$id.'">Delete</a>
                                    </td>
                                </tr>
                            </thead>';           
                        }
...
<?php 
    if($department_name == 'Admin'){
        ...
        <script>
            ...
            "Student LifeLong | Apply by Students";
        </script>'; 
    }   
    else if($department_name == 'Admission'){
