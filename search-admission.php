    ...
    <div class="container mt-5 mb-5">
        <div class="row justify-content-center">
            <div class="col-xl-4 col-lg-4 col-md-5 col-sm-6 col-8">
                <form class="form-bg" action="next-search-admission.php" method="post">
                    <div class="mb-3">
                        <h4 class="text-center field-top">Admission</h4>
                    </div>
                    <div class="mb-3">
                        <input class="form-control" type="text" list="idsuggestions" 
                        id="suggestions" name="suggestions" placeholder="Search Admission No" 
                        title="Search Admission No" onkeyup="noSpace(this)" 
                                ...
                                if($result=$mysqli->query($qury)){
                                    if(mysqli_num_rows($result)>0){
                                        while($row=$result->fetch_assoc()){
                                            echo '<option value="'.$row['admission_id'].'">'
                                            .$row['admission_id'].'</option>';
                                        }
                                    }
                                }
                            ?>
