<?php 
    ...
    $qury = "SELECT COUNT(*) AS id FROM contact_us";
    if($result=$mysqli->query($qury)){
        if(mysqli_num_rows($result)>0){
            $row = $result->fetch_assoc();
            $total_contact = $row['id'];
        }
    }
?>
...
<div class="container mt-5">
  <div class="row justify-content-center mt-3">
      ...
      <div class="card home">
        <div class="self-card-body text-center">
          <h5 class="card-title">Reach to Us : <?php echo $total_contact;?></h5>
          <a href="try-to-contact-by-visitors.php" class="btn btn-primary btn-learn">View</a>
        </div>
      </div>
  </div>
</div>
